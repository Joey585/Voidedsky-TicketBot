...
function createTicket(interaction){
    ...
    interaction.guild.channels.create({
        ...
        channel.setTopic("ticket ID: " + id).then(() => {
            const newTicket = new TicketChannel({ id: id, creatorID: interaction.user.id.toString(), type: type, timeCreated: Math.round(Date.now() / 1000), lastTalked: null, participants: {}, closed: false, reason: "", closeUserId: ""})
            newTicket.save().then(() => {
            ...
            Guild.findOne({id: interaction.guild.id}, (err, guild) => {
                if(guild){
                    guild.tickets.push(`${channel.name}-${id}`);
                    guild.save();
                }
            })
...
module.exports = { createTicket, TicketChannel, ticketChannelSchema, guildSchema, Guild };